...
const SECONDS_IN_WEEKS = 604800;
const SECONDS_IN_YEARS = 31536000;
      ...
      geojson: Record({
        features: Array(
          Record({
            ...
            geometry: Record({
              type: Literal("Point"),
              coordinates: Array(Number).withConstraint(
                (val) => val.length === 2
              ),
            }).Or(
              Record({
                ...
                coordinates: Array(
                  Array(Array(Number).withConstraint((val) => val.length === 2))
                ),
              })
            ),
          })
...
function convertGeometryToPoints(
  geometry: ZoneList["paringzones"][number]["geojson"]["features"][number]["geometry"]
): [number, number][] {
...
function convertPeriodToSeconds(period: string): number | undefined {
  ...
  const months = /(?<months>\d+) MONTHS?/;
  const weeks = /(?<weeks>\d+) WEEKS?/;
  const years = /(?<years>\d+) YEARS?/;
  ...
  const weeksMatch = weeks.exec(period);
  if (weeksMatch !== null) {
    return parseInt(weeksMatch.groups!.weeks!, 10) * SECONDS_IN_WEEKS;
  }

  const monthsMatch = months.exec(period);
  if (monthsMatch !== null) {
    return parseInt(monthsMatch.groups!.months!, 10) * SECONDS_IN_MONTHS;
  }

  const yearsMatch = years.exec(period);
  if (yearsMatch !== null) {
    return parseInt(yearsMatch.groups!.years!, 10) * SECONDS_IN_YEARS;
  }
...
function convertTimestamp(timestamp: string): number | undefined {
  const match = /(?<hours>\d{1,2}):(?<minutes>\d{2}):(?<seconds>\d{2})/.exec(
    timestamp
  );
  if (match === null) {
    return undefined;
  ...
  return (
    parseInt(match.groups!.hours!, 10) * SECONDS_IN_HOURS +
    parseInt(match.groups!.minutes!, 10) * SECONDS_IN_MINUTES +
...
main()
  .then((result) =>
    fs.writeFileSync("europark.json", JSON.stringify(result, null, 2))
  )
  .catch(console.error);
